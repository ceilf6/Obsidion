# 1. eagerState - Hook dispatch

https://github.com/ceilf6/Lab/commit/359eec8d4d5251fd94d0c4984c76527c006e5de3

在 Hook 的 dispatch 也就是挂载阶段的 dispatchSetState 中有

```jsx
if (queue.lastRenderedReducer !== null) {
  const eagerState = queue.lastRenderedReducer(
    lastRenderedState,
    action
  );
...
直接用当前 **reducer** 计算一次 **nextState**，如果不变，那么连 ScheduleUpdateOnFiber 都不走

只在 useState / useReducer 生效

Vue 在**编译**时可以进行性能优化，但是 React 不行，React 是**运行**时JSX+普通JS函数，只能通过暴露 性能优化相关API 给开发者

React 19 开始 **React Compiler（以前叫 React Forget）**会自动 useMemo, useCallback
...
- 在第二次打印的时候，并没有打印 child render，此时实际上是命中了 **bailout** 策略。命中该策略的组件的**子组件会跳过 reconcile 过程**，
也就是说**子组件不会进入 render 阶段**。
- 后面的第三次以及之后的点击，没有任何输入，说明 App、Child 都没有进入 render 阶段，此时命中的就是 **eagerState** 策略，这是一种发生于触发状态更新时的优化策略，如果命中了该策略，此次**更新不会进入 schedule 阶段，更不会进入 render 阶段**。

## eagerState 策略

该策略的逻辑其实是很简单：如果**某个状态更新前后没有变化，那么就可以直接使用缓存的上一次状态，跳过后续的更新流程**。

**state 是基于 update 计算出来的，计算过程发生在 render 的 beginWork**，而 eagerState 则是将计算过程**提前到了 schedule 之前执行**。
...
> - bailout 策略
> 
> eagerState 的核心逻辑是如果**某个状态更新前后没有变化**，则可以跳过后续的更新流程。该策略将**状态的计算提前到了 schedule 阶段之前**。当有 FiberNode 命中 eagerState 策略后，就**不会再进入 schedule 阶段，直接使用上一次的状态**。
> 
> 该策略有一个前提条件，那就是当前的 FiberNode 不存在待执行的更新，因为如果**不存在待执行的更新，当前的更新就是第一个更新**，计算出来的 state 即便不能命中 eagerState，也能够在后面作为**基础 state** 来使用，这就是为什么 FC 所使用的 **Update 数据中有 hasEagerState 以及 eagerState 字段的原因。**
>