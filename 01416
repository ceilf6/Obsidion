...
completeWork (reconciler)
      ↓
调用 createInstance (来自 hostConfig)
      ↓
具体 renderer 实现
```

宿主环境 ⇒ hostConfig ⇒ Fiber 算法和平台无关

## mount 阶段

https://github.com/ceilf6/Lab/commit/f5d99560957ce17e113b982c5b097ae876ade500
...
- 深度优先 - 从当前的 FiberNode **向下遍历**，将遍历到的第一层 DOM 元素类型（HostComponent、HostText）通过 appendChild 方法插入到 parent 末尾
- 对**兄弟 FiberNode 执行步骤 1**
- 如果没有兄弟 FiberNode，则**对父 FiberNode 的兄弟执行步骤 1**
...
appendAllChildren 方法实际上就是在**处理下一级的 DOM 元素**，而且在 appendAllChildren 里面的遍历过程会更复杂一些，会多一些判断，因为 FiberNode 最终形成的 FiberTree 的层次和最终 DOMTree 的层次可能是有区别：
...
在上面的代码中，如果从 **FiberNode** 的角度来看，Hello 和 World 是同级的，但是如果从 **DOM** 元素的角度来看，Hello 就和 span 是同级别的。因此从 FiberNode 中查找同级的 DOM 元素的时候，经常会涉及到**跨 FiberNode 层级**进行查找。

### 3. finalizeInitialChildren

接下来 completeWork 会执行 finalizeInitialChildren 方法完成**属性的初始化**，主要包含以下几类属性：
...
上面的 mount 流程，完成的是属性的初始化，那么这个 update 流程，完成的就是**属性更新**的**标记**

Reconciler 只是标记，到 commit 统一处理

updateHostComponent 的主要逻辑是在 diffProperties 方法里面，这个方法会包含**两次遍历**：

- 第一次遍历，主要是标记**更新前有，更新后没有**的属性，实际上也就是标记**删除**了的属性
- 第二次遍历，主要是标记**更新前后有变化**的属性，实际上也就是标记**更新了的**属性
...
新版顺序是第一次找更新，第二次找删除（可以 Set 一次遍历即可，但是 React 为了极致的性能）
...
所有更新了的属性的 key 和 value 会保存在当前 FiberNode**.updateQueue** 里面，数据是**以 key、value 作为数组相邻的两项**的形式进行保存的
...
我们知道，当整个 Reconciler 完成工作后，会得到一颗完整的 **wipFiberTree**，这颗 wipFiberTree 是由一颗一颗 FiberNode 组成的，这些 FiberNode 中有一些标记了 flags，有一些没有标记，现在就存在一个问题，我们如何高效的找到散落在这颗 wipFiberTree 中**有 flag 标记的 FiberNode**，那么此时就可以通过 flags 冒泡。

我们知道，completeWork 是属于**归**的阶段，整体流程是自下往上，就非常适合用来**收集副作用**，收集的相关的代码如下：
...
这样的收集方式，有一个好处，在渲染阶段，通过任意一级的 FiberNode.subtreeFlags 都可以**快速确定该 FiberNode 以及子树是否存在副作用**从而**判断是否需要执行和副作用相关的操作**。
