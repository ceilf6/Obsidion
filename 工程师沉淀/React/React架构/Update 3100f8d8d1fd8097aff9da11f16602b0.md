...
https://github.com/ceilf6/Lab/commit/4e701d52bbf27ca3b15996ad6f0d6d596b7a04c2
...
虽然这些方法执行的场景会有所不同，但是都可以**接入同样的更新流程**，原因是因为它们**使用同一种数据结构来表示更新**，这种数据结构就是 **Update**。

## Update 数据结构

在 React 中，**更新**是存在**优先级**的（ I/O响应 ），其心智模型有一些类似于“代码版本管理工具”。

举个例子，假设现在我们在开发一个软件，当前软件处于正常的迭代中，拥有 A、B、C 三个正常需求，此时突然来了一个紧急的线上 Bug，整体流程如下：

![image.png](工程师沉淀/React/React架构/Update/image.png)
...
![image.png](工程师沉淀/React/React架构/Update/image%201.png)

当 Bug 修复完毕后，再正常的来迭代 A、B、C 需求，之后的迭代会基于 D 这个版本：

![image.png](工程师沉淀/React/React架构/Update/image%202.png)
...
在上面的 Update 数据结构中，tag 字段是用于区分**触发更新的场景**的，选项包括：
...
在上面的数据结构中，有 hasEagerState 和 eagerState 这两个字段，它们和后面要介绍的 React 内部的**性能优化**策略（**eagerState 策略**）相关。
...
- 更新的**紧急程度**：紧急程度是由 **lane** 字段来表示的
- 更新之间的**顺序**：通过 **next** 字段来指向下一个 update，从而形成一个链表。

## UpdateQueue

上面所介绍的 **update 是计算 state 的最小单位**，updateQueue 是由 update 组成的一个**链表**，挂载在 hook对象上，在 updateReducerImpl 中进行遍历、执行
...
- baseState：参与计算的初始**基准** state，update 基于该 state 计算新的 state，可以类比为心智模型中的 **master 分支**。
...
![image.png](工程师沉淀/React/React架构/Update/image%203.png)

之后进入新的一轮 render，在该 FiberNode 的 beginWork 中，shared.pending 所指向的环状链表就会被拆分，拆分断开之后接入到 baseUpdate 链表后面：

![image.png](工程师沉淀/React/React架构/Update/image%204.png)

接下来就会**遍历 ( do…while )** updateQueue**.baseUpdate**，基于 updateQueue.baseState 来计算每个**符合优先级条件**的 update（这个过程有点类似于 Array.prototype.**reduce**），最终计算出最新的 state，该 state 被称之为 memoizedState。

![image.png](工程师沉淀/React/React架构/Update/image%205.png)
