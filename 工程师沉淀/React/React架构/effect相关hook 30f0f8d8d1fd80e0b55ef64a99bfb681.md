...
https://github.com/ceilf6/Lab/commit/6d3dbcefa6db709010caa1fd82e0174877725111

// 如果不包在 useEffect 中，会导致 fetch => setData => reRender => fetch => 无限循环

https://github.com/ceilf6/Lab/commit/9804504cf993254fc5a5cf1116e16f421909fabb
...
- useEffect：回调函数会在 **commit 阶段完成后异步执行**，所以它**不会阻塞视图渲染**
    
    **Passive** Effect
    
- useLayoutEffect：回调函数会在 **commit 阶段的 Layout 子阶段同步执行**，一般用于**执行 DOM 相关**的操作
    
    **Layout** Effect
    
- useInsertionEffect：回调函数会在 **commit 阶段的 Mutation 子阶段同步执行**，与 useLayoutEffect 的区别在于执行的时候**无法访问对 DOM 的引用**。这个 Hook 是**专门为 CSS-in-JS 库插入全局的 style 元素**而设计。
...
next 字段会与**当前的函数组件的其他 effect 形成环状链表，连接的方式是一个单向环状链表**。
...
- 声明阶段
- **调度**阶段（只有 useEffect 是**异步**的，是他独有的）
- 执行阶段
...
**如果deps依赖项发生改变，就会打上 effectTag ，在commit节点，就会再次执行我们的effect**，之后会推入新的 effect 到环状链表上面。
...
**flushPassiveEffects** 会去**执行**对应的 **effects**：
...
之所以使用 **do...while 循环**，就是为了**保证上一轮调度的 effect 都执行过了**。

React 18 改动

目标：passive effects 不能阻塞 commit

passive effect 即 useEffect 这种不会阻塞 DOM 提交和浏览器绘制的副作用

1. 移除了 do..while ，不再 commit → 立刻循环 flush
2. 改成了调度模型
...
- commitHookEffectListUnmount ：用于**遍历** effect 链表依次执行 effect.**destory** 方法
...
由于 commitHookEffectList**Unmount** 方法的执行时机会先于 commitHookEffectList**Mount** 方法执行，**因此每次都是先执行 effect.destory 后才会执行 effect.create**。
...
> 
> - useEffect：回调函数会在 **commit 阶段完成后异步执行**，所以**不会阻塞视图渲染**
> - use**Layout**Effect：回调函数会在 **commit 阶段的 Layout 子阶段同步执行**，一般用于执行 **DOM 相关**的操作
> 
> 每一个 effect 会与当前 FC 其他的 effect 形成环状链表，连接方式为**单向环状链表**。
> 
