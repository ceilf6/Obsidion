...
https://github.com/ceilf6/Lab/commit/fcaa3aad117f02515b4fab201b2a5166d1918818
...
- HooksDispatcher**OnMount**：负责**初始化**工作，让函数组件的一些初始化信息**挂载到 Fiber 上面**
...
判断了是mount还是update之后，会给 ReactCurrentDispatcher.current 赋值对应的 dispatcher，因为**赋值了不同的上下文对象，因此就可以根据不同上下文对象调用不同的方法**。
...
renderWithHooks 会被每次函数组件触发时（mount、update），该方法就会清空 workInProgress 的 memoizedState 以及 updateQueue，

接下来判断该组件究竟是初始化还是更新，为 ReactCurrentDispatcher.current **赋值**不同的上下文对象，之后调用

Component 方法来执行函数组件，组件里面所书写的 hook 就会依次执行。

最后调用 finishRenderingHooks 在其中上下文赋值为 ContextOnlyDispatcher 防止 hooks 在函数组件外部调用，如果调用直接报错
...
许多 FiberNode 通过链表串联用于表示 虚拟树

hook 链表用于实现一个 FiberNode函数组件的状态变更
...
![image.png](Hooks%E5%8E%9F%E7%90%86/image.png)
...
这里**如果 nextWorkInProgressHook 不为 null，那么就会复用之前的 hook**，这里其实也就解释了为什么 **hook 不能放在条件或者循环语句里面**

**复用 *hooks* 状态和当前 *hooks* 不一致的问题**

> 面试题：hook 为什么通常放在顶部，而且不能写在条件或者循环语句里面？
> 
> 
> 因为更新的过程中，如果通过 if 条件**增加或者删除了 hook**，那么在**复用Hook**的时候，就会产生当前hook 的顺序和之前 hook 的顺序不一致的问题。
> 
...
![image.png](Hooks%E5%8E%9F%E7%90%86/image%201.png)

第二次复用的时候，发现 hook 的类型不同， useState !==useRef，那么就会直接报错。因此开发的时候一定要注意 hook 顺序的一致性。

![image.png](Hooks%E5%8E%9F%E7%90%86/image%202.png)
...
> 
> 在更新时，会通过 **hook链** **复用**之前的 Hook，如果hook写在了 *if* 条件语句，导致增加或者删除 *hooks*，那么**hook链的顺序被打乱**，在复用 *hooks* 过程中，会产生**复用 *hooks* 状态和当前 *hooks* 不一致的问题**。
>