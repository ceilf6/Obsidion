...
https://github.com/ceilf6/Lab/commit/fb2fd3b0287f22fff529e270c5dbf8f783e44098
...
> [https://react.docschina.org/docs/faq-internals.html](https://react.docschina.org/docs/faq-internals.html)
> 
- **Virtual DOM 是一种编程概念**。在这个概念里， UI 以一种理想化的，或者说“虚拟的”表现形式被保存于**内存**中。

也就是说，只要我们有一种方式，能够将真实 DOM 的层次结构描述出来，那么这就是一个虚拟 DOM。

![image.png](工程师沉淀/React/React架构/虚拟%20DOM/image.png)

在 React 中，React 团队使用的是 JS 对象来对 DOM 结构进行一个描述。但是很多人会直接把 JS 对象和虚拟 DOM 划等号，这种理解是不太准确的，比较片面的。

**虚拟 DOM 和 JS 对象**之间的关系：**前者是一种思想，后者是一种思想的具体实现。**
...
首先我们需要明确一个点，**JS** 层面的**计算**速度要比 DOM 层面的计算要快：
...
虚拟 DOM 在第一次渲染页面的时候，并没有什么优势，速度肯定比直接操作原生 DOM API 要慢一些，虚拟 DOM 真正体现优势是在**更新**阶段。
...
虚拟 DOM 只是多真实 UI 的一个描述，回头**根据不同的宿主环境，可以执行不同的渲染代码**：
...
React17 之后不再使用 React.createElement，改为使用 react/jsx-runtime 中的 jsx() / jsxs()
...
在上面的代码中，最终返回的 element 对象就是我们所说的**虚拟 DOM 对象**。在官方文档中，官方更倾向于将这个对象称之为 **React 元素**。
...
> 
> 在 React 中，使用了 **JS 对象**来描述真实的 DOM 结构。虚拟DOM和 JS 对象之间的关系：前者是一种**思想**，后者是这种思想的具体实现。
> 
> 使用虚拟 DOM 有如下的优点：
> 
> - **相较于 真实DOM** 的体积和速度优势
...
> - ReactTest 包用于渲染出 JS 对象，可以很方便地测试“不隶属于任何宿主环境的通用功能”
> 
> 在 React 中，通过 JSX 来描述 UI，JSX 仅仅是一个语法糖，会被 [**Babel**](https://babeljs.io/repl) 编译为 createElement 方法的调用。**该方法调用之后会返回一个 JS 对象，该对象就是虚拟 DOM 对象**，官方更倾向于称之为一个 React 元素。
>