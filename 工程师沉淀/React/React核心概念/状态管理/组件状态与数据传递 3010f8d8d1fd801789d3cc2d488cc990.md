...
通过 *props.**children***，可以实现类似于 *Vue* 中 **插槽** 的功能
...
在 *Vue* 中，可以对传入的 *props* 设置默认值 default ，以及验证 *props* 的有效性 validate ，在 *React* 中，针对 *props* 也可以做这些事。
...
关于 *props* 的类型检查，从 *React v15.5* 版本开始，移入到了
...
vue中子组件想更新父组件数据得通过emit上报事件，React中子组件想更新父组件数据是直接通过：父组件传递**父组件中的更新函数**到子组件props中让其调用

其实**更新函数就是事件**，本质都是**父组件掌握状态、子组件只负责通知、触发更新**
