...
通过**`justify-content`**属性，可以影响**主轴的排列方式**

space- 控制**空白**剩余空间围绕元素的排布

![image-20210511113617325](http://mdrs.yuanjin.tech/img/20210511113617.png)

### 侧轴排列

通过**`align-items`**属性，可以影响**侧轴的排列方式**

![image-20210511114016304](http://mdrs.yuanjin.tech/img/20210511114016.png)

以上属性都是用在 弹性盒子包含块元素 上的

![image.png](工程师沉淀/CSS/CSS3/image.png)

### 弹性项目伸缩

所谓伸缩，是指在**主轴方向**上，
当**弹性容器**有**额外空间**时，是否需要拉伸，当**空间不足**时，是否需要**压缩**

在 **弹性盒中的元素** 即 **弹性项目item** 上使用`flex`属性，可设置拉伸和压缩比例：
`flex: 拉伸比例 压缩比例 初始尺寸`
...
当 flex 为 auto 时和 flex: 1 的区别在于 基准尺寸flex-basis: auto 依元素自身内容而定
...
虽然 flex 布局会导致像 float, clear 等属性失效，但是弹性元素的基础属性还是能有效的，例如

- margin
    
    像 margin: auto 能居中，margin-left 可以**推到当前 flex 行中剩余可用空间的最右侧**
...
默认情况，当**主轴剩余空间不足时，按照压缩比例进行压缩**，但如果设置了主轴换行，则不会压缩，直接换行显示

给**弹性容器**设置`flex-wrap: wrap`，即可主轴换行

![image.png](工程师沉淀/CSS/CSS3/image%201.png)
...
![image.png](工程师沉淀/CSS/CSS3/image%202.png)

容器生成网格布局后，其所有子元素为**网格元素** 或称为 **网格项目item**

### fr单位

fraction 分数

**1fr = “把剩余空间平均分成若干份中的一份”**
...
### 改变排列方向 **grid-auto-flow**

使用属性**`grid-auto-flow**: column`，可使子元素按列排放

![image.png](工程师沉淀/CSS/CSS3/image%203.png)
...
默认情况下，网格项目在**单元格内部**水平和垂直拉伸，以撑满单元格
...
![image.png](工程师沉淀/CSS/CSS3/image%204.png)
...
但可以对 **网格项目** 设置`grid-area`属性来改变这一行为
...
![image.png](工程师沉淀/CSS/CSS3/image%205.png)

# 视觉

> 所谓**视觉类样式**，是指**不影响盒子位置、尺寸的样式**，例如阴影、文字颜色、背景颜色、背景图片等
> 
...
[http://mdrs.yuanjin.tech/html/css-manual/box-shadow.html?v=2](http://mdrs.yuanjin.tech/html/css-manual/box-shadow.html?v=2)

![image.png](工程师沉淀/CSS/CSS3/image%206.png)
...
[http://mdrs.yuanjin.tech/html/css-manual/text-shadow.html?v=3](http://mdrs.yuanjin.tech/html/css-manual/text-shadow.html?v=3)

![image.png](工程师沉淀/CSS/CSS3/image%207.png)
...
[http://mdrs.yuanjin.tech/html/css-manual/border-raduis.html?v=5](http://mdrs.yuanjin.tech/html/css-manual/border-raduis.html?v=5)

![image.png](工程师沉淀/CSS/CSS3/image%208.png)
...
在设置**背景图片**时，除了可以使用`url()`加载一张背景图，还可以使用**`linear-gradient()`函数**设置背景渐变
...
**transform 不是在渲染主线程中，不会导致浏览器 reflow 和 rerender，因此效率极高**
...
[http://mdrs.yuanjin.tech/html/css-manual/rotate.html](http://mdrs.yuanjin.tech/html/css-manual/rotate.html)
...
[http://mdrs.yuanjin.tech/html/css-manual/transform-origin.html?v2](http://mdrs.yuanjin.tech/html/css-manual/transform-origin.html?v2)
...
注意：旋转会导致**坐标系也跟着旋转，从而可能影响到后续的变形效果**
...
**过渡和动画无法对所有的CSS属性产生影响，能够产生影响的只有 数值类属性**，例如：颜色、宽高、字体大小等等
...
使用**`border-box`根据边框控制尺寸更加直观**，因此，很多网站都会加入下面的代码
...
![image.png](工程师沉淀/CSS/CSS3/image%209.png)

使用伪元素可以避免在HTML中使用过多的空元素

**伪元素必须要有`content`属性，否则不能生效，如果不需要有元素内容，设置`content:''` （例如在清除浮动时）**
