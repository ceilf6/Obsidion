...
如果这样做，客户端可以随便填写一个别人的手机号，然后**从 cookie 中获取到验证码，从而绕开整个验证**。

因此，有些**敏感数据是万万不能发送给客户端的**

那要如何实现这一流程呢？

![image-20210914161657162](http://mdrs.yuanjin.tech/img/20210914161657.png)

**包一层 sid - sessionid 给 cookie，而不是直接发敏感数据**
...
> 参考答案：
> 
> 1. cookie 的数据保存在**浏览器端**；session 的数据保存在**服务器**
...
> 2. **客户端主动通知**
>     
>     可以使用 JS 监听客户端页面关闭 **window.unload** 或其他退出操作，然后通知服务器清除 session
>