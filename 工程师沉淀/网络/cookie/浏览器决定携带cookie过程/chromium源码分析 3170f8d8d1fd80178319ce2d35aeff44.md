...
[**默认情况下，ajax 的跨域请求并不会附带 cookie**，这样一来，某些需要权限的操作就无法进行](同源策略%202f60f8d8d1fd805aa2add4064d274431.md) 

---

## 浏览器决定携带哪些 Cookie 的源码分析

整个决策链涉及 4 个核心文件，按调用顺序展开：
...
**文件**url_request_http_job.cc L2096](Browser/Chromium-source/net/url_request/url_request_http_job.cc#L2096)
...
**文件**url_request_http_job.cc L838](Browser/Chromium-source/net/url_request/url_request_http_job.cc#L838)

**实现**cookie_util.cc L895](Browser/Chromium-source/net/cookies/cookie_util.cc#L895)
...
`ComputeSameSiteContextForRequest` 的核心逻辑（[cookie_util.cc L895](https://www.notion.so/Browser/Chromium-source/net/cookies/cookie_util.cc#L895)）按 RFC6265bis 规范计算上下文等级：

| 上下文类型 | 条件 |
| --- | --- |
| `SAME_SITE_STRICT` | URL、initiator、site_for_cookies 三者注册域相同 |
| `SAME_SITE_LAX` | URL 与 site_for_cookies 注册域相同 + 主框架导航 + 安全方法(GET/HEAD) |
| `SAME_SITE_LAX_METHOD_UNSAFE` | 同上但方法不安全(POST等) |
| `CROSS_SITE` | 其他情况 |
...
**文件**cookie_monster.cc L764](Browser/Chromium-source/net/cookies/cookie_monster.cc#L764)
...
**文件**cookie_base.cc L218](Browser/Chromium-source/net/cookies/cookie_base.cc#L218)
...
### 检查3：Scheme 绑定（`kSchemeBoundCookies` Feature）
...
### 检查4：Port 绑定（`kPortBoundCookies` Feature）
...
### 检查5：Domain 匹配 — `IsDomainMatch()`

**文件**cookie_util.cc L704](Browser/Chromium-source/net/cookies/cookie_util.cc#L704)
...
### 检查6：Path 匹配 — `IsOnPath()`

**文件**cookie_util.cc L737](Browser/Chromium-source/net/cookies/cookie_util.cc#L737)
...
**文件**url_request_http_job.cc L854](Browser/Chromium-source/net/url_request/url_request_http_job.cc#L854)
