...
https://github.com/ceilf6/Lab/commit/bcadabcdf485eff395a109c814578c9aed4c0ed1
...
场景：登陆状态、网站全局设置、用户便好设置

且在组件层级复杂、数量众多的时候，知道数据是从什么状态变化到什么状态，方便调试
...
- 某个组件改动数据后，如何**让其他用到该数据的组件知道数据变化**了？
    - 事件总线貌似可以解决该问题，但需要在组件中手动的维护监听，极其不方便，而且**事件总线的目的在于「通知」，而不是「共享数据」**

一种比较容易想到的方案，就是把所有的共享数据**全部**提升到根组件，然后通过属性**不断下发**，当某个组件需要修改数据时，又不断向上抛出事件，直到根组件完成对数据的修改。

![image-20210126133905451](http://mdrs.yuanjin.tech/img/20210126133905.png)

这种方案的缺陷也非常明显：

- 需要编写大量的代码层层**下发**数据，很多组件**被迫拥有了自己根本不需要的数据**
- 需要编写大量的代码层层**上抛**事件，很多组件**被迫注册**了自己根本处理不了的事件
...
- 响应式：组件可以自由的改变仓库中的数据，仓库的数据变化后，会自动通知用到对应数据的组件更新
    
    “Invoke property state”
...
- 特别注意：
1. `mutation`中**不得出现异步操作**
    
    > 在实际开发的规范中，甚至要求**不得有副作用操作**
    > 
    > 
    > 副作用操作包括：
    > 
    ...
    因为 vuex 会在受到 **commit** 运行对应 mutation 方法之后**立即记录状态**，并不会等待事件循环运行副作用方法结束
    
2. **commit 提交是数据改变的唯一原因**
    
    从而方便监控
    

![image.png](%E6%95%B0%E6%8D%AE%E5%85%B1%E4%BA%AB%20vuex/image.png)

# 异步处理

如果在`vuex`中要进行异步操作，需要使用`action`

因为所有数据变更只可能来自 commit ，所以在 action 中变更数据是通过 commit

触发 commit 的过程叫做 dispatch 分发，到时候通过

store.dispatch( ) 触发

![image.png](%E6%95%B0%E6%8D%AE%E5%85%B1%E4%BA%AB%20vuex/image%201.png)
