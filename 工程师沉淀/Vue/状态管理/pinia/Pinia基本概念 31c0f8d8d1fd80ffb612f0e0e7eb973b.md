...
pinia 官网：[https://pinia.vuejs.org/](https://pinia.vuejs.org/)

![image.png](Pinia%E5%9F%BA%E6%9C%AC%E6%A6%82%E5%BF%B5/image.png)
...
export const useCounterStore = defineStore('counter',{ // 仓库名称
  state: () => ({
    count: 0
  }),
  getters: {
...
1. 可以和 **TypeScript** 一起使用，以此来获得**类型**推断的支持
...
2. 支持插件扩展，可以通过**插件（函数）**来扩展仓库的功能，为仓库添加全局属性或者全局方法

```jsx
// ...
// 这里定义了一个名为 localStoragePlugin 的插件，本质上就是一个**函数**
...
1. 更加**轻量**，压缩之后体积只有 1kb 左右，基本上可以忽略这个库的存在
...
> - **actions 中支持同步和异步**方法修改 state 状态。
