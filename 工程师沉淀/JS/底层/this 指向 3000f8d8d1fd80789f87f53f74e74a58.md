...
https://github.com/ceilf6/Lab/commit/633867a28243d7f6dd8912d7c7f283d73c634eee

**字节码（Bytecode） = 虚拟机可以执行的中间指令**
...
解释器执行字节码
   ↓
热点代码交给 TurboFan 编译成机器码
```

[真题](this%20%E6%8C%87%E5%90%91/%E7%9C%9F%E9%A2%98%203190f8d8d1fd806b9820ce090380ba20.md)
...
[**在调用前，谁向寄存器写值**](this%20%E6%8C%87%E5%90%91%203000f8d8d1fd80789f87f53f74e74a58.md) 

[并且 普通函数的 this 取决于调用方式](../../React/React%E6%A0%B8%E5%BF%83%E6%A6%82%E5%BF%B5/%E7%B1%BB%E7%BB%84%E4%BB%B6/this%E4%B8%A2%E5%A4%B1%E9%97%AE%E9%A2%98%2031e0f8d8d1fd802cba60c810f54538cd.md) 

    **箭头**函数的 this 不是调用时绑定、而是**词法绑定的**，其指向不看运行时调用方式，而看其**定义位置的外层**

所以**箭头函数调用 call, apply, bind 的时候没法改变箭头函数的 this**

在编译期就：[箭头函数被**显式排除**，这是"箭头函数没有自己的 `this`"的根本原因。](this%20%E6%8C%87%E5%90%91%203000f8d8d1fd80789f87f53f74e74a58.md) 

显式绑定中 call 参数需要一个个传；apply 参数直接传一个数组（call长度4短于apply，参数需要展开补短 doge）
...
`this` 的处理横跨**编译期**和**运行期**，分四个层次。

---

### Layer 1 — 编译期：声明 `this` 变量

[scopes.cc](http://scopes.cc/) — `DeclareThis()`
...
void DeclarationScope::DeclareThis(AstValueFactory* ast_value_factory) {
  ...
  if (derived_constructor) {
    receiver_->ForceHoleInitialization(...);      // 强制打洞
  }
  locals_.Add(receiver_);  // 加入局部变量列表
}
```

**谁会调用它？** [scopes.cc](http://scopes.cc/)

```cpp
set_has_this_declaration(
    (is_function_scope() && **!is_arrow_scope()**)  // 函数作用域，但排除箭头函数
    || is_module_scope()
);
```

箭头函数被**显式排除**，这是"箭头函数没有自己的 `this`"的根本原因。

---

## Layer 2 — 运行期：`this` 存在哪里？

`this` 的存储位置有三种情况，取决于变量分配阶段的结果：

### 情况 A：普通函数（`VariableLocation::PARAMETER`）
...
`this` 被视为一个 index=-1 的特殊**参数**，存在**栈帧的 Receiver 寄存器**里，而不是普通参数列表里。

### 情况 B：`this` 被闭包捕获（`VariableLocation::CONTEXT`）

若内层函数引用了外层函数的 `this`，分配阶段调用 `ForceContextAllocation()`，**强制把 `this` 提升到 Context 堆槽**。

[bytecode-generator.cc](http://bytecode-generator.cc/) — 函数入口时把 Receiver 寄存器的值写入 Context slot：
...
if (scope->has_this_declaration() && scope->receiver()->IsContextSlot()) {
  ...
  builder()
    ...
    .StoreContextSlot(execution_context()->reg(), variable, 0);
  //                  ^^^^^^^^^^^^^^^^^^^^^^^^^^
  //                  把栈帧里的 this 值，存入 Context[slot_index]
}
```

### 情况 C：箭头函数（没有 `this` 声明）

**不会为箭头函数调用 `DeclareThis`**，所以它**根本没有 `receiver_` 变量。读取 `this` 时直接往外找**。

---

## Layer 3 — 读取 `this`：`VisitThisExpression` 调用链

[bytecode-generator.cc](http://bytecode-generator.cc/)

```
AST ThisExpression 节点
    ↓
VisitThisExpression(expr)
    ↓
BuildThisVariableLoad()
    ↓
GetReceiverScope()          ← 关键：沿作用域向上找，跳过箭头函数作用域
    ↓
BuildVariableLoad(var, HoleCheckMode)
    ↓
  [PARAMETER 分支] variable->IsReceiver() == true
    → builder()->Receiver()      ← 加载栈帧 Receiver 寄存器
  [CONTEXT 分支]
    → depth = ContextChainDepth(scope)
    → LoadContextSlot(context_reg, variable, depth)   ← 沿 Context 链跳 depth 层
```

`GetReceiverScope` 的实现（[scopes.cc](http://scopes.cc/)）：
...
这个 while 循环**跳过所有箭头函数作用域**，直到找到有 `has_this_declaration` 的非箭头函数作用域——这正是"箭头函数的 `this` 是词法 `this`"的底层实现机制。

---

## Layer 4 — 派生类构造函数的 TDZ
...
`kRequired` 会触发 [bytecode-generator.cc](http://bytecode-generator.cc/)，而且因为 `variable->is_this() == true`，抛出的不是普通的 `ReferenceError`，而是专用字节码 `ThrowSuperNotCalledIfHole`，对应错误信息：
...
class Foo extends Bar {
  constructor() {
    this.x = 1;   ← 访问 this（派生类）
    super();
    this.y = 2;   ← super() 之后，hole 被填充，访问合法
  }
}
```
...
| 阶段 | 操作 | 源码位置 |
| --- | --- | --- |
| Parse | 识别 `class extends`，标记 `IsDerivedConstructor` | `src/parsing/parser.cc` |
| Scope Analysis | `DeclareThis` 创建 `kConst` + `kNeedsInitialization` + `ForceHoleInitialization` | `scopes.cc#L759` |
| Variable Allocation | `this` 分配到 `PARAMETER`(index=-1) 或 `CONTEXT` slot | [scopes.cc](http://scopes.cc/) resolver |
| Bytecode Gen | 函数入口不初始化 `this`（保持 hole 值） | [bytecode-generator.cc](http://bytecode-generator.cc/) |
| `this` 读取 | `GetReceiverScope` → `BuildVariableLoad` → `HoleCheckMode::kRequired` → `ThrowSuperNotCalledIfHole` 字节码 | `bytecode-generator.cc#L8167` |
| `super()` 调用 | 调用父类构造函数，返回值写入 `this` slot，填充 hole | `BuildSuperCallOptimization` |
...
**DeclareThis** / `BuildThisVariableLoad` 只是**读**这个寄存器（或 Context slot），四规则差异全部发生在**写**的阶段——即 **`VisitCall`** / `VisitCallNew` 的 call site

**四规则不是 V8 设计的 API**，而是 JavaScript 程序员对"四条不同的 call site 编译路径"所归纳出的认知模型。底层实现里没有"四规则"这个概念，有的只是**四条不同的字节码**（`Construct` / `CallAnyReceiver` / `CallProperty` / `CallUndefinedReceiver`），每条字节码向 Receiver 寄存器写入的来源不同。

### 1. 默认绑定 - CallUndefinedReceiver
...
```jsx
    ...
    builder()->CallUndefinedReceiver(callee, args, ...);
    // ↑ 专用字节码：receiver 槽为空，运行时填 undefined(严格) 或 global(宽松)
}
```

`foo()` → `CallUndefinedReceiver` 字节码 → 运行时决定是 `undefined` 还是全局对象（**严格模式**切换点就在这个字节码的运行时实现里）。
...
```cpp
    ...
    // ↑ 专用字节码：args[0] 就是 receiver，即点前面的对象
}
```

`obj.foo()` → 编译时把 `obj` 推入 args[0] → `CallProperty` 字节码 → receiver = obj。

---

### 3. 显式绑定 - CallAnyReceiver

`foo.call(thisArg, ...)` 在 bytecode-generator 眼里仍然是一次 `NAMED_PROPERTY_CALL`（调用 `foo` 上的 `call` 属性），走的是 `CallProperty` 路径，但 **`Function.prototype.call` 这个内置函数**本身会：取出 `arguments[0]` 作为 receiver，然后以 `CallAnyReceiver` 再次调用 `foo`：

```cpp
// 最终落到这个分支（CallAnyReceiver = 接受任意显式 receiver）
} else {
    builder()->CallAnyReceiver(callee, args, ...);
}
```

`bind` 更彻底：返回一个 **BoundFunction 对象**，其内部持有固定 receiver，后续任何调用均走 `CallAnyReceiver`，且 receiver 已在对象创建时**锁死**。

显式绑定的"强制性"表现为：无论外部怎么调用（`CallProperty` / `CallUndefinedReceiver`），内置 `call`/`apply`/`bind` 都会**覆盖 receiver 写入**。
...
void BytecodeGenerator::VisitCallNew(CallNew* expr) {
    ...
    //         ^^^^^^^^^ 专用字节码，完全独立于 VisitCall
}
```

`Construct` 字节码在运行时做的事：

1. 分配新对象（`[[Prototype]]` 设为 `Constructor.prototype`）
2. 把新对象写入 Receiver 寄存器（即 `this`）
3. 调用构造函数体
4. 若返回值是对象，用返回值替换新对象；否则保留新对象

`new` 走的是完全独立的代码路径（`VisitCallNew` + `Construct` 字节码），这是它能**覆盖一切**的原因——连 `VisitCall` 的 switch 都不经过。

---

### 优先级从何而来？

写入的时机，越后面的就会覆盖前面写入的、呈现出来就是优先级更高
...
new     → VisitCallNew → Construct 字节码   → 运行时分配新对象**强制**写 receiver
显式    → 内置 call/apply/bind             → **覆盖** receiver 再调用 CallAnyReceiver
隐式    → VisitCall NAMED/KEYED_PROPERTY   → **编译时** obj 压入 args[0] → CallProperty
默认    → VisitCall GLOBAL/OTHER_CALL      → implicit_undefined_receiver=true → CallUndefinedReceiver
```
...
- 在**箭头**函数中，*this* 的指向是由**外层（函数或全局）作用域来决定的**。
...
<img src="[https://xiejie-typora.oss-cn-chengdu.aliyuncs.com/2021-09-28-033304.png](https://xiejie-typora.oss-cn-chengdu.aliyuncs.com/2021-09-28-033304.png)" alt="image-20210928113303839" style="zoom:50%;" />
...
但是，*forEach* 方法的回调函数内部的 [*this.name*](http://this.name/) 却是指向全局对象，导致没有办法取到值。稍微改动一下，就可以看得更清楚。
...
<img src="[https://xiejie-typora.oss-cn-chengdu.aliyuncs.com/2021-09-28-052059.png](https://xiejie-typora.oss-cn-chengdu.aliyuncs.com/2021-09-28-052059.png)" alt="image-20210928132058878" style="zoom:50%;" />
...
[函数表达式和this](this%20%E6%8C%87%E5%90%91/%E5%87%BD%E6%95%B0%E8%A1%A8%E8%BE%BE%E5%BC%8F%E5%92%8Cthis%202fc0f8d8d1fd80c29da4e49d3879340f.md)