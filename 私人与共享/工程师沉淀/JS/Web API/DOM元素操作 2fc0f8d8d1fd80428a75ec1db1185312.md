...
事件处理程序：是一个函数，发生了一件事，应该做什么事情 - 回调函数
...
- innerHTML：获取和设置元素的内部HTML文本
    ...
    // getElement 返回的是集合
    divEle[0].innerHTML = '<div style="color:red">I\'m ceilf6</div>'
    
    // 或者直接用 querySelector
    const div = document.querySelector('div');
    div.innerHTML = '<div style="color:red">I\'m ceilf6</div>';
    ```
    
    ![image.png](DOM%E5%85%83%E7%B4%A0%E6%93%8D%E4%BD%9C/image.png)
    
    ![image.png](DOM%E5%85%83%E7%B4%A0%E6%93%8D%E4%BD%9C/image%201.png)
    
- innerText：获取和设置元素内部的纯文本，仅得到元素内部显示出来的文本
- **textContent**：获取和设置元素内部的**纯文本**，textContent得到的是内部源代码中的文本
    
    不要用 innerHTML，而是用 textContent **只进行文本渲染**，防止 **XSS**
